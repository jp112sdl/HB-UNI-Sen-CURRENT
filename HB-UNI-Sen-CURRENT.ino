...
class MeasureEventMsg : public Message {
  public:
    void init(uint8_t msgcnt, uint8_t channel, uint16_t current) {
      Message::init(0x0e, msgcnt, 0x53, BIDI | WKMEUP, channel & 0xff, (current >> 8) & 0xff);
      pload[0] = current & 0xff;
    }
};
...
class MeasureChannel : public Channel<Hal, UList1, EmptyList, List4, PEERS_PER_CHANNEL, UList0>, public Alarm {
  ...
  public:
    MeasureChannel () : Channel(), Alarm(0), current(0) {}
    ...
    virtual void trigger (__attribute__ ((unused)) AlarmClock& clock) {
      ...
      tick = delay();
      msg.init(msgcnt, number(), current);
